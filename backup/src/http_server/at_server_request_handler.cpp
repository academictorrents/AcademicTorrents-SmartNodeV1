...
//		  cout << std::string(collection_id[0].first, collection_id[0].second);
		  ...
		  vector<vector<string> > results =
				  db->query("SELECT DISTINCT T.type, T.name, T.infohash, T.sizebytes, T.mirrors, T.downloaders, T.timescompleted, T.dateadded, T.datemodified, T.status, T.filename, T.bibtex FROM torrents T JOIN Collections2Torrents C ON T.infohash=C.infohash WHERE C.urlname=\"" + collection_id + "\";");
		  std::vector<string> col = db->getColNames();
		  db->close();
