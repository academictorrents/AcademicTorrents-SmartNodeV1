...
//		Database *db = new Database();
	...
	vector < vector<string> > results =
			db->query(
					"SELECT DISTINCT T.infohash from Torrents T JOIN Collections2Torrents CT ON T.infohash=CT.infohash WHERE CT.urlname=\""
							+ collection + "\" AND T.torrentpath=\"NULL\";");
	//db->close();
...
void at_fetcher::parse_collection_list(const std::string path) {
	boost::asio::io_service io_service;
	async_at_client::async_at_connection c(io_service, AT_URL, path,
			write_collections_list, "");
	io_service.run();
}
